...
SELECT last_name, count(last_name) AS '# of Actors'
FROM actor
GROUP BY 1 HAVING count(last_name) > 1;
...
-- 		popularity. Use subqueries to display the titles of movies starting with the letters K and Q whose
-- 		language is English.
...
JOIN country co ON (co.country_id = cty.country_id)
...
SELECT s.store_id, cty.city, cou.country 
FROM store s
JOIN address a ON (s.address_id = a.address_id)
JOIN city cty ON (cty.city_id = a.city_id)
JOIN country cou ON (cou.country_id = cty.country_id);
